...
import dd.crawler.codeforce.db.VisitedPageSql;
import dd.crawler.codeforce.meta.Pair;
import dd.crawler.codeforce.meta.Status;
import dd.crawler.codeforce.statusparser.StatusParser;
	...
	private void runCrawler() throws Exception {
		...
		while (true) {
			...
			try {
				...
				if (list.size() == 50) {
					VisitedPageSql.insert(pageId); //最后再插入
				} else {
